    ...
    public RestTemplate restTemplate() {
        ...
        restTemplate.getInterceptors().add((request, body, execution) -> {
            ...
            if (attributes instanceof org.springframework.web.context.request.ServletRequestAttributes) {
                // If there is an active web request, cascade its session headers
                ...
                // Background task (Kafka Consumer) has no active web request
                // We inject a SYSTEM ADMIN role so it can fetch the necessary info
                request.getHeaders().add("X-User-Role", "ADMIN");
                request.getHeaders().add("X-User-Id", "9999");
            }
